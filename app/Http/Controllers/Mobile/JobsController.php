...
class JobsController extends Controller
{
        ...
        if (isset($job)) {
            ...
            return new JsonResponse([
                'response' => [
                    ...
                    'details' => 'Job with the provided job id does not exist',
                ],
            ], 400);
        }
    ...
    public function deleteProjectAsset(Request $request)
    {
        ...
        if (isset($asset)) {
            $jobId = $asset->job_listing_id;
            if ($asset->delete()) {
                return new JsonResponse([
                    'response' => [
                        "message" => 'Successful',
                        "files_and_images" => JobAssets::where("job_listing_id", $jobId)->get(),
                    ],
                ], 200);
            } else {
                return new JsonResponse([
                    'response' => [
                        'message' => 'Unable To Delete Job Asset',
            ...
            return new JsonResponse([
                'response' => [
                    'message' => 'Unable To Delete Job Asset',
                    'details' => 'Asset with the provided asset id does not exist',
                ],
            ], 400);
        }
    }
        ...
        if (isset($tcExist)) {
            return new JsonResponse([
                'response' => [
                    ...
                    'details' => 'Please generate new tracking code. The tracking code provided is already assigned.',
                ],
            ], 400);
        }
        ...
        try {
            ...
            if (!isset($business)) {
                return new JsonResponse([
                    'response' => [
                        'message' => 'Unable To Create Job',
                        'details' => 'No active business found for this account',
                    ],
                ], 400);
            }
            ...
            $job->country_iso = $request->country_iso;
